...

{
	...
	for (j = 0; j < prod; j++)
	{
		...
		//tabu[j] = (int**)malloc(sizeof(int*)*prod);
		for (i = 0; i < prod; i++)
		{
			ic1[j][i] = -1;
			ic2[j][i] = -1;
			//tabu[j][i] = (int*)malloc(sizeof(int)*client);
		}
	...
	//initialized = false;
	while (iter_no_improvement < 30 && time_elapsed <= time_limit ) /* loop for the tabu search until a stopping criteria be found */
	{
		...
		if (1)
		{
			...
			for (i1 = 0; i1 < client; i1++)
			{
				...
				{//Testar se os orcamentos permitem fazer a troca simples antes de guardar na memoria (tambem testar se mcpo e mantido). Nao se esquecer de ajustar a soma de ofertas do produto nesse caso
					...
					{	//If the offer record in ic1 frees enought budget to allow the current swap to be performed, perform both swaps
						if ((tabu[j2][j1][ic1[j2][j1]] < iter && tabu[j1][j2][i] < iter) || !flag_tabu)
							...
							{//ic1[j2][j1] guarda uma troca beneficial de uma oferta do produto j2 para o j1
								//ofertar o produto j1 para o cliente ic1[j1][j2], removendo a oferta deste cliente para o produto j2
								//ofertar o produto j2 para o cliente i e remover a oferta o produto j1 para este cliente
								...
								//printf("Bestmove: alt Tabu_cost: %f Best_Solution: %d\n", tabu_cost, best_solution);
								//check_budget(tabu_solc, budgetpo, residual_budget, cost, prod, client);
								...
								//talvez o cliente tenha servido de vetor para outra troca
							...
							{
								//ofertar o produto j1 para o cliente ic2[j1][j2], removendo a oferta deste cliente para o produto j2
								//ofertar o produto j2 para o cliente i e remover a oferta o produto j1 para este cliente
								...
								//printf("Bestmove: alt2 Tabu_cost: %f Best_Solution: %d\n", tabu_cost, best_solution);
								ic2[j2][j1] = -1;
								better_solution_found = true;
								if (flag_tabu)
								{
							...
							else if (ic2[j1][j2] >= 0 && (revenue[i][j2] - cost[i][j2] >= revenue[i][j1] - cost[i][j1]) && client_po[j1] > mcpo[j1])//Rever as condicoes	
							{
								/*if((cost[i][j1] > cost[ic2[j1][j2]][j1] && cost[i][j2] >= cost[ic2[j1][j2]][j2]) || (cost[i][j2] > cost[ic2[j1][j2]][j2] && cost[i][j1] >= cost[ic2[j1][j2]][j1]))
									ic2[j1][j2] = i;*/
						...
						//testar custo e ver se troca e viavel em relacao aos orcamentos (testar se tem oferta em ic2)
						//Se for possivel, fazer a troca
						//Senao, acumular essa nova oferta em ic2
					}
		...
		/*compute_solution_value(client, prod, hurdle,
			cost, revenue, maxofer,
			mcpo, budgetpo, fcostpo,
			solc, solp, solution_cost, prod_can);*/
													...
													/* do customer[i1][j]=0 and customer[i2][j]=1 that improves the solution value */
		{
			...
			if (tabu_solp[j1] != 0)
			{
				...
				for (aux = sorted_npp[j1]->prox, i_ = 0; (i_ < client - 1 && iteration_counter < interval*client); i_++, aux = aux->prox)
				{
					...
					{
						//--Seguir do NPP parando sempre no cliente i1
						iteration_counter++;
													 ...
													 /* do customer[i1][j1]=0 and customer[i2][j1]=1  and customer[i1][j2]=1 and customer[i2][j2]=0 that improves the solution value */
		{
			...
			if (tabu_solp[j1] != 0)
			{
				...
				{
					//j2 = npp_sorted_prod_list[j2_];
					//Se o produto j2 esta aberto
					if ((tabu_solp[j2] != 0) && (j1 != j2))
					{
						//--Ir por ordem de NPP em relacao a J1
						for (aux = sorted_npp[j1]->prox; (aux); aux = aux->prox)
						{
							i1 = aux->inf;
							//--Ir por ordem de NPP em relacao a J2
											...
											((1 + hurdle)*((*sum_cost_fix) + sum_cost_dif - cost[i1][j1] + cost[i2][j1] + cost[i1][j2] - cost[i2][j2])))))
										{
												...
												+ ((revenue[i1][j2] - cost[i1][j2]) - (revenue[i2][j2] - cost[i2][j2])))))
											{
											   ...
											   // flag_move2 = 1;
												...
												tabu_cost += best_solution;
												//printf("Bestmove: %d Tabu_cost: %f Best_Solution: %d\n", best_move, tabu_cost, best_solution);
												/*printf("Clients: (%d, %d)   Products: (%d,%d).\n", i1, i2, j1, j2);*/
												incumbent_solution = tabu_cost;
