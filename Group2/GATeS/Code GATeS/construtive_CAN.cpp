	...
	List** sorted_npp, int target_prod)
{
	...
	//Criar uma lista de ofertas para o produto alvo
	//Ordenar essa lista por ordem crescente de custo
	...
	//Inserir ofertas para o produto em ordem crescente de custo ate satisfazer o numero minimo de ofertas
	...
	//Procedimento heuristico
	//Realizar trocas de ofertas realizadas seguindo a ordem decrescente de npp para ofertas removidas e crescente de npp para ofertas adicionadas, respeitando o orcamento e limites de ofertas para clientes
	//Parar quando o hurdle for atingido
	for(int tries=0;tries<5;tries++)
	for (i = 0; i < mcpo[target_prod]; i++)
	{
		/*if (revenue[offer_set[i]][target_prod] / cost[offer_set[i]][target_prod] < 1.0 + hurdle  )
		{*/
...
int* create_variance_vector(int **cost, int **revenue, int client, int prod, int *solp)
{
	...
	/*if (Variance_vector == NULL)
	{
		Variance_vector = (int*)malloc(sizeof(int)*client);
	}*/
	/*if (!aux_vector)
	{
		aux_vector = (float*)malloc(sizeof(float)*client);
	}*/
...
int** create_cvp_npp_matrix(int** cost, int** revenue, int prod, int client)
{
	...
	/*if (!CvP_npp)
	{
		CvP_npp = generate_2D_matrix_int(client, prod);
	}*/
	/*if (!aux_vector)
	{
		aux_vector = (float*)malloc(sizeof(float)*prod);
	}*/
	...
	for(i = 0; i < client; i++)
	{
		...
		//printf("Order for client %d\n", i);
		/*for (j = 0; j < prod; j++)
		{
			printf("(%f,%d);", aux_vector[CvP_npp[i][j]], CvP_npp[i][j]);
		}
		printf("\n");*/
	}
...
//The attempt of resolving conflicts consists on fiding a valid offer to replace each offer on the offer set being evaluated that causes conflict. The algorithm tries to pick the swap that would cause less loss of profit. 
bool resolve_conflicts(int **conflict_matrix, int **solc, int *solp, int conflicted_prod, int prod, int client, int **cost, int **revenue, int *fcostpo, int *maxofer, int *mcpo, int *budgetpo, List **sorted_npp, int *sum_offer_c, int *sum_offer_p, int* sum_offer_r, int **offer_reserve, int *reserve_cost, int* prod_cost, int *prod_rev, int *residual_budget, int *conflicts)
{
	...
	//criar um aux para lista de reserva e troca-la definitivamente apenas se conflitos forem retirados com sucesso
		...
		if (aux_solc[c->inf][conflicted_prod])
		{
			...
			if (aux_sum_offer_c[c->inf] > maxofer[c->inf])
			{
				...
				for (j = 0, less_impacted_product = -1, block_impact = INT_MAX; j < prod && prod_buffer[j] >= 0; j++)
				{
					...
					if (aux_sum_offer_p[prod_buffer[j]] > mcpo[prod_buffer[j]] /*&& replacement==-1*/)
					 {
						 if ((revenue[c->inf][prod_buffer[j]] - cost[c->inf][prod_buffer[j]])/**(aux_sum_offer_p[prod_buffer[j]]/mcpo[prod_buffer[j]])*/ < block_impact)
						 {
				...
				else
				{
					if (replacement >= 0)
					{
						...
						/*if (less_impacted_product != conflicted_prod)
						{*/
	...
	fclose(log);
	return true;
	//Criar uma copia de solc
	//Para cada cliente ofertado para o produto conflituoso
		//Verificar se o cliente gera coflito (sum_offer_c > maxofer)
			//Se o cliente gerar conflito
				//Para cada produto ja pertencente a solucao que foi ofertado para o cliente em questao
					//Avaliar impacto no lucro do produto da remocao do cliente em relacao ao lucro medio por oferta
					//Avaliar se a remocao do cliente inviabiliza o produto (manter referencia ao menor impacto que mantenha viabilidade)
				//Se houver produto que possa ter a oferta do cliente removida
					//Remover a oferta do cliente de menor impacto
				//Caso nao exista produto que possa ter a oferta removida mantendo viabilidade, retornar falso pois o produto nao pode ser inserido
}
	...
	float* sum_cost_fix, float* sum_profit, int* prod_can, int target_prod)
{
	...
	for (i = 0; i < mcpo[target_prod]; i++)
	{
		...
		{*/
			...
			for (aux_list = sorted_npp[target_prod]->prox; aux_list; aux_list = aux_list->prox)
			{
				...
				{
					/*printf("(%d,%d); ", offer_set[i], aux_list->inf);
					if (aux_list->inf == 0)
						printf("Aviso");*/
					selected_offers[offer_set[i]] = false;
					offer_cost += cost[aux_list->inf][target_prod] - cost[offer_set[i]][target_prod];
	...
	if (i < client)
		offer_set[i] = -1;
	/*printf("\n Set de ofertas final para o produto %d \n", target_prod);
	for (i = 0; i < mcpo[target_prod]; i++)
	{
		printf("%d; ", offer_set[i]);
	}*/
	//inserir o conjunto de ofertas no vetor offer_set
}
	...
	float* sum_cost_fix, float* sum_profit, int* prod_can)
{
	...
	for (j = 0; j < prod; j++)
	{
		...
		{
			if (cost[product_aux[j]->inf][j] + reserve_cost[j] <= residual_budget[j] + 4*budgetpo[j] && (revenue[product_aux[j]->inf][j]/(cost[product_aux[j]->inf][j]/*+profit_dilution*/)) >= hurdle)
			{
		...
		if (solp[prod_index[j]] > 0)
		{
			...
			for (product_aux[prod_index[j]] = sorted_npp[prod_index[j]]->prox; product_aux[prod_index[j]] && residual_budget[prod_index[j]]>=minimum_cost[prod_index[j]]; product_aux[prod_index[j]] = product_aux[prod_index[j]]->prox)
			{
				if (cost[product_aux[prod_index[j]]->inf][prod_index[j]] <= residual_budget[prod_index[j]] && (revenue[product_aux[prod_index[j]]->inf][prod_index[j]])/(cost[product_aux[prod_index[j]]->inf][prod_index[j]]) >= hurdle && sum_offers_c[product_aux[prod_index[j]]->inf] < maxofer[product_aux[prod_index[j]]->inf] && (solc[product_aux[prod_index[j]]->inf][prod_index[j]]<1))
				{
					...
					//selected_client = product_aux[prod_index[j]]->inf;
	...
	int* prod_can, int popSize, int initial_pos)
{
	...
	for (int c = initial_pos; c < popSize; c++)
	{
		...
		for (j = 0; j < prod; j++)
		{
			...
			for (i = 0, reserve_cost[j] = 0, sum_offer_r[j] = 0; product_aux[j] && !finished[j]; product_aux[j] = product_aux[j]->prox)
			{
				if (cost[product_aux[j]->inf][j] + reserve_cost[j] <= n->budget[j] + 4 * budgetpo[j] && (revenue[product_aux[j]->inf][j] / (cost[product_aux[j]->inf][j]/*+profit_dilution*/)) >= hurdle)
				{
		...
		for (j = 0; j < prod; j++)
		{
			if (sum_offers_p[prod_index[j]] >= mcpo[prod_index[j]])
			{
				//Criar uma versao de resolve conflicts para cromossomos
	...
	float* sum_cost_fix, float* sum_profit, int* prod_can, int iter)
{
	...
	while (search_clients)
	{
		...
		for (j = 0; j < prod; j++)
		{
			...
			if (active_products[j] != 0)
			{
				...
				cont = 0;
				last_offer_counter = 0;
				last_offer = aux_list->inf;//remover
				iter_no_offer = 0;
				//Modificar esse loop por um loop randomizante
				List *aux_list2 = sorted_npp[j];//remover
				cost_avrg = 0;
				while (cont < mcpo[j] && sum_cost[j] < budgetpo[j])
				{
					random = (rand() % interval) + 1;
					flag = false;
					for (aux_list = aux_list2, cont2 = 0; aux_list->prox; aux_list = aux_list->prox)//Utilizar vetor ao inves da lista
					{
						if (!(selected_clients[aux_list->inf]) && active_clients[aux_list->inf] > 0)
						{
					...
					//Talvez entre num ponto onde nao existem mais ofertas que nao estourem o orcamento, tentando repetir sempre a mesma oferta que vai estourar o orcamento
					if (active_clients[aux_list->inf] > 0 && sum_cost[j] + cost[aux_list->inf][j] <= budgetpo[j] && selected_clients[aux_list->inf] < 1)//Utilizar vetor ao inves de lista
					{
						sum_cost[j] += cost[aux_list->inf][j];
						sum_revenue[j] += revenue[aux_list->inf][j];
						aux_allocated_clients[cont] = aux_list->inf;
				...
				if (sum_cost[j] + 3 >= budgetpo[j] /*&& cont < mcpo[j]*/)
				{
					...
					while (cost_avrg > cost_trgt && cont<10)
					{
						for (j1 = 0, contador_custo = 0; j1 < client && aux_allocated_clients[j1] != -1; j1++)
						{
							if (cost[aux_allocated_clients[j1]][j] > cost_trgt)
							{
								...
								//Criar uma lista binaria de clients selecionados para o produto para que possa ser feita uma checagem rápida de quais clientes foram selecionados
								//durante o processo de remanejamento de ofertas.
								if (aux_list->inf == initial_node->inf && !(active_clients[aux_list->inf] > 0))//Utilizar vetor ao inves de lista
									aux_list = NULL;
								if (aux_list)//Usar vetor ao inves de lista
								{
					...
					{
						for (aux_list = sorted_npp[j]->prox; aux_list; aux_list = aux_list->prox)//Utilizar vetor ao inves de lista
						{
							cost_avrg2 = ((float)(sum_cost[j] + cost[aux_list->inf][j]) / (offers[j] + 1));
			...
			((val + sum_revenue[index_max_pr]) >= ((1 + hurdle)*(exp + sum_cost[index_max_pr])))*/)
		{
			...
			for (cont = 0; cont < mcpo[index_max_pr]; cont++)
			{
				//**Verificar os valores finais do residual budget
				/*printf("%d, ", allocated_clients[cont]);*/
				solc[allocated_clients[cont]][index_max_pr] = 1;
				residual_budget[index_max_pr] -= cost[allocated_clients[cont]][index_max_pr];
				active_clients[allocated_clients[cont]] -= 1;
	...
	while (search_clients)
	{
		...
		//***Enxer a stack antes (acima do mcpo) antes de inserir o fcost
	...
	List** sorted_npp, int* prod_can, int popSize, float range, float hurdleTolerance, float penalty, float *average_npp)
{
	...
	for (c = 0, prod_quant = prod; c < popSize; c++)
	{
		...
		/*for (i = 0; i < client; i++)
			for (j = 0; j < prod; j++)
				pop[c].clients[i].offers[j] = false;*/
		if(c>0)
			for (i = 0; i < prod; i++)
			{
				if (rand() % 100 < 15)
		...
		while (search_clients)
		{
			...
			for (j = 0, prod_cont = 0; j < prod; j++)
			{
				...
				if (active_products[j] != 0 && !ignored_prod[j] && !blockedProd[j])
				{
					...
					cont = 0;
					last_offer_counter = 0;
					last_offer = cli_order[0][j]/*aux_list->inf*/;
					iter_no_offer = 0;
					rand_flag = true;
					interval = mcpo[j];
					//Modificar esse loop por um loop randomizante
					List *aux_list2 = sorted_npp[j]->prox;//remover
					while (cont < mcpo[j] && sum_cost[j] + min_cost < budgetpo[j])
					{
						if (rand_flag)
							random = (rand() % interval) + 1;
						flag = false;
						/*for (aux_list = aux_list2, cont2 = 0; aux_list->prox && cont2 < random; aux_list = aux_list->prox, cont2++);*/
						for (/*aux_list = aux_list2, cont2 = 0*/cont2=random; /*aux_list->prox*/ cont2<client; /*aux_list = aux_list->prox,*/ cont2++)
						{
							if (!(selected_clients[cli_order[cont2][j]/*aux_list->inf*/]) && pop[c].clients[cli_order[cont2][j]/*aux_list->inf*/].offerSlots > 0 && cost[cli_order[cont2][j]][j]+sum_cost[j] <= budgetpo[j])
								break;
							if (cont2 == client-1)
							{
								cont2 = 0;
							...
							if (cont2 == random - 1)
								break;
							/*if (!(selected_clients[aux_list->inf]) && pop[c].clients[aux_list->inf].offerSlots > 0)
							{
								if (flag == false)
								{
									flag = true;
									...
									if (random == 1)
									{
										for (aux_list2 = aux_list2->prox; aux_list2 && selected_clients[aux_list2->inf]; aux_list2 = aux_list2->prox);
									}
								}
								cont2++;
							}
						...
						if (pop[c].clients[cli_order[cont2][j]/*aux_list->inf*/].offerSlots > 0 && sum_cost[j] + cost[cli_order[cont2][j]/*aux_list->inf*/][j] <= budgetpo[j] && !selected_clients[cli_order[cont2][j]/*aux_list->inf*/] && pop[c].clients[cli_order[cont2][j]/*aux_list->inf*/].offers[j] == 0)
						{
							sum_cost[j] += cost[cli_order[cont2][j]/*aux_list->inf*/][j];
							sum_revenue[j] += revenue[cli_order[cont2][j]/*aux_list->inf*/][j];
							aux_allocated_clients[cont] = cli_order[cont2][j]/*aux_list->inf*/;
							cont++;
							selected_clients[cli_order[cont2][j]/*aux_list->inf*/] = 1;
							last_offer = cli_order[cont2][j]/*aux_list->inf*/;
							last_offer_counter = 0;
							iter_no_offer = 0;
						}
						else
						{
							if (cli_order[cont2][j]/*aux_list->inf*/ != last_offer)
							{
								last_offer = cli_order[cont2][j]/*aux_list->inf*/;
								last_offer_counter = 0;
							}
							...
							iter_no_offer++;
							if (!pop[c].clients[cli_order[cont2][j]/*aux_list->inf*/].offerSlots) break;
							/*if (cost[aux_list2->inf][j] + sum_cost[j] <= budgetpo[j])
							{
								rand_flag = false;
								random = 1;
							}
								...
								break;*/
							//if (iter_no_offer > (interval*0.30))
							//{
							//	rand_flag = false;
							//	//break;
							//}
							if(iter_no_offer > 1/*(interval*0.05)*/)
							{
								break;
							}
						}
		...
		//check_budget(&pop[c], budgetpo, cost, prod, client);
		//check_values(&pop[c], fcostpo, cost, revenue, pop[c].cost, pop[c].revenue, pop[c].fitness, prod, client);
...
//		if (sum_offers_p[prod_index[j]] >= mcpo[prod_index[j]])
//		{
//			if (resolve_conflicts(conflict_matrix, solc, solp, prod_index[j], prod, client, cost, revenue, fcostpo, maxofer, mcpo, budgetpo, sorted_npp, sum_offers_c, sum_offers_p, sum_offer_r, offer_reserve, reserve_cost, prod_cost, prod_rev, residual_budget, conflicts))
//			{
//				solp[prod_index[j]] = 1;
//				/*compute_solution_value(client, prod, hurdle,
//					cost, revenue, maxofer,
...
//			{
//				//Criar uma versao de resolve conflicts para cromossomos
//				if (resolve_conflicts_GEN(conflict_matrix, n, prod_index[j], prod, client, cost, revenue, fcostpo, maxofer, mcpo, budgetpo, sorted_npp, sum_offers_c, sum_offers_p, sum_offer_r, offer_reserve, reserve_cost, prod_cost, prod_rev, conflicts))
//				{
//					n->offeredProducts[prod_index[j]] = true;
//					/*compute_solution_value(client, prod, hurdle,
//						cost, revenue, maxofer,
...
//	int **cost, int **revenue, int* maxofer,
//	int* mcpo, int* budgetpo, int* fcostpo,
//	int **solc, int* solp, float* solution_cost, int* prod_can)
//
//{
//	int i, j;
//	float cost1, revenue1;
...
//		{
//
//			if (active_products[j] != 0 && (!can1 || active_products[prod_can[j]] <= 0))
//			{
//				aux_list = sorted_npp[j]->prox;
//				cont = 0;
//				heuristic_constructive(client, prod, hurdle, cost, revenue, maxofer, mcpo, budgetpo, fcostpo, aux_allocated_clients, sorted_npp, active_clients, active_products, sum_cost_fix, sum_profit, prod_can, j);
...
//}
////Builds an population for the genetic algorithm with an randomized greedy constructive algorithm.
//void build_population(int client, int prod, float hurdle,
//	int **cost, int **revenue, int* maxofer,
//	int* mcpo, int* budgetpo, int* fcostpo,
//	cromo *pop,
//	List** sorted_npp, int* prod_can, int popSize, float range, float hurdleTolerance, float penalty, float *average_npp)
...
//		}
//	}*/
//	//*********** last nao esta sendo updatadoS
//
//	for (j = 0; j < prod; j++)
//	{
//		for (aux_list = sorted_npp[j]->prox, pred = NULL, i = 0; aux_list; aux_list = aux_list->prox, i++)
//		{
//			aux_list->ant = pred;
//			cli_order[i][j] = aux_list->inf;
//			/*if (aux_list->prox == NULL)
