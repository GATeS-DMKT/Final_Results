...
int main(void)
{
	...
	for (i = 0; i < POPSIZE; i++)
	{
		//pop1[i].budget = (int*)malloc(sizeof(int)*prod);
	...
	for (i = 0; i < POPSIZE; i++)
	{
		//pop2[i].budget = (int*)malloc(sizeof(int)*prod);
		for (i2 = 0; i2 < MAXPROD; i2++)
		...
		/*if (!initialized)
		{*/
		/*pop2[i].offeredProducts = (bool*)malloc(sizeof(bool)*prod);*/
	/*}*/
		for (j = 0; j < MAXPROD; j++)
		...
		for (h = 0;h < 1;h++)
		{
			for (l = 0;l < 1;l++)
			{
				for (int i1 = 0;i1 < 1;i1++)
				{
					...
					for (j = 0; j < MAXPROD; j++)
						prod_can[j] = -100;
					//Instance that will be processed
					...
					for (i2 = 0, best_solution_cost=0, global_best=0;i2 < resets;i2++)
					{
						...
						//printf("Ordem de produtos: \n");
						for (j = 0; j < prod; j++)
						{
							active_products[j] = 1;
							//printf("(%d, %d);", npp_ordered_product_list[j], prod_order[j]);
							for (i = 0; i < client; i++)
							{
								active_clients[i] = maxofer[i];
								solc[i][j] = 0;
						...
						if (i2==0)
						{
							build_initial_solution(client, prod, hurdle,
								...
								&sum_cost_fix, &sum_profit, prod_can,prod_order); //Enviar ponteiros para vetores ordenados
						...
						{	//If it is the second run, builds the solution with the alternative constructive algorithm
							if (i2 == 1 )
							{
								alternative_constructive_algorithm(client, prod, hurdle,
									...
									mcpo, budgetpo, fcostpo,
									solc, solp, sorted_npp, active_clients, active_products,
									&sum_cost_fix, &sum_profit, prod_can); //Enviar ponteiros de vetores ordenados
								compute_solution_value(client, prod, hurdle,
									cost, revenue, maxofer,
									mcpo, budgetpo, fcostpo,
									solc, solp, &solution_cost, prod_can,false);
						...
						{				
							...
							/*time_tabu += (end_tabu - begin_alg) / CLOCKS_PER_SEC;*/
