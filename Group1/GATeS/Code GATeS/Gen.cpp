...
void mutate_matrix(int prod, int client, bool *offered_products)
{
	int i, j, random;
	...
	for (i = 0; i < prod; i++)
		for (j = 0; j < client; j++)
		{
			mutation_matrix[j][i] = false;
		}
	for (i = 0, j = 0; i < (int)(client*0.20); i++)
	{
		random = rand() % client;
		if (j == prod)
			j = 0;
		for (; !offered_products[j]; j++)
		{
			if (j == prod)
				j = 0;
		}
		if (mutation_matrix[random][j])
		{
			i--;
			continue;
		}
		mutation_matrix[random][j] = true;
	}
}
...
//checks most commonly made offers in the current population
...
//applies crossover operator with canibalism taken into consideration. It will for non cannibalism instances if the canibalism flag is false.
int crossover_CAN(cromo *p1, cromo *p2, cromo *n, int prod, int cli, int* mcpo, int totalOffers, int** revenue, int** cost, int** profit, list **sorted_npp, int *maxOffer, int *oBudget, float hurdle, float penaltie, int *fcpo, int *client_order, int* prod_order, int* prod_can, bool mutation)
{
	...
	//Para isso, considerar que produtos canibais nao ocupam slots de ofertas nos clientes (ainda devendo respeitar restricoes de orcamento)
	//No final do processo, varrer a solucao e decidir qual canibal adotar considerando seu lucro total  em clientes onde
	//o lucro de sua oferta supere o de produtos que estao efetivamente ocupando slots de ofertas
	//Apos esse processo, realizar um segundo processo de complementacao da solucao
	...
	{
		cont = client_order[k];
		//checks each clients offerset and compares the profit of the feasible offers
		...
		//remover as ofertas da contagem caso o produto nao possa atingir o mcpo
		//Comeca o processo de insercao das ofertas realizada na configuracao de cliente mais lucrativa entre os dois pais.
		//Adds the feasible offers from the winning gene to the new chromosome 
		if (profit1 > profit2)
		{

			//Checar orcamento antes de inserir e atualizar os valores de custo e lucro caso alguma oferta seja removido
			for (i = 0, cont2 = 0, moffer = false; i < prod; i++)
			{
				//Se o produto i foi ofertado.
				if (offers[i] >= 0)
				{
					//Se ainda for possivel para o produto atingir sua cota de ofertas, considerando a quantidade de clientes remanescentes; e se a oferta couber no orcamento do produto.
		...
		else
		{
			...
			//Realiza as ofertas presentes na configuracao do cliente caso elas sejam possiveis de serem realizadas e ainda seja possivel que o produto atinja sua cota de ofertas.
				...
				if (offers[i] >= 0)
				{
					...
					else
					{
						if (p2->clients[cont].offers[i])
						{
							...
							if (offers[i] + offerCount < mcpo[i])
							{
								//Caso um produto seja contatado com inviavel,  realiza uma varredura por todos os produtos e
								//remove o produto mais longe de atingir sua cota da solucao e o marca como inviavel.
								...
								//Se o produto marcado como inviavel foi ofertado na configuracao de cliente selecionada, reavalia as configuracoes
								//de ofertas dos dois pais para o mesmo cliente.
	...
	//Passo2: O passo 2 cosiste no reparo da solucao inicial
	//Percorre-se a lista de produtos, primeiramente indentificando os produtos eliminados durante a primeira etapa por nao serem capazes de cumprir sua cota
	//Para esses produtos, e realizado o processo de limpeza das ofertas marcadas previamente a eliminacao do produto
	//Ao se indentificar produtos que foram ofertados(e nao foram eliminados), verifica-se se sua cota foi atingida
	//Caso a cota desse produtos ainda nao foi atingida, e iniciado o processo de preenchimento, percorrendo-se os clientes por ordem de npp para o produto
	//E ofertando-os, caso as restricoes permitam, ate que a cota seja atingida. Se a cota nao for atingida apos este processo, elimina-se o produto.
	...
	for (i = 0; i < prod; i++)
	{
		...
		{
			//printf("Foi iniciado o processo de complementacao de ofertas do prodduto %d, com um orcamento inicial de %d.\n", i, n->budget[i]);
			for (aux = sorted_npp[i]->prox; aux && offers[i] < mcpo[i]; aux = aux->prox)
			{
				if (n->clients[aux->inf].offerSlots > 0 && n->budget[i] >= cost[aux->inf][i] && n->clients[aux->inf].offers[i] == 0 && (!mutation_matrix[aux->inf][i] || !mutation))
				{
					...
					offers[i]++;
					//printf("-Oferta inserida para o produto %d, cliente %d. Custo: %d Orcamento restante: %d \n", i, aux->inf, cost[aux->inf][i], n->budget[i]);
					//n->clients[aux->inf].npp = n->clients[aux->inf].revenue / n->clients[aux->inf].cost;
				}
			}
			if (offers[i] < mcpo[i])
			{
				//printf("Entrou no procedimento de complementacao.\n");
				offers[i] = -1;
				n->budget[i] = oBudget[i];
				n->offeredProducts[i] = false;
				if (offers[i] < 0)
	...
	//Passo 3: Preenchimento da solucao
	//O preenchimento consiste em duas etapas, a primeira e o preenchimento relativo a genes nao selecionados no processo inicial de atribuicao de ofertas.
	//Essa primeira etapa tem como objetivo utilizar-se o maximo de informacoes relativas as duas solucoes pais para o processo de construcao da solucao filha
	//A segunda etapa consiste no preenchimento tradicional do construtivo, percorrendo-se cada um dos produtos ofertados e preenchendo duas ofertas ate o limite de seu orcamento com clientes ordenados por NPP
	//Fill the resulting solution with offers that belong to the losing genes during the first part of the crossover
	for (i = 0; i < cli; i++)
	{
	...
	//****Gerando um loop de solucoes zeradas. Checar.
...
int reproduction_CAN(cromo *pop1, cromo *pop2, int prod, int cli, int* mcpo, int totalOffers, int** revenue, int** cost, int** profit, list **sorted_npp, int *maxOffer, int *oBudget, float hurdle, float penaltie, int tam, float *probability, int *fcpo, cromo *elite, int *client_order, int *prod_order, int* prod_can)
{
	...
	//**Falta mutacao e mandar referencia do elite apos a reproducao e receber referencia do elite da populacao original
	//Reproducao e realizada por roleta ponderada
	for (int j = 0; j < tam; j++)
	{
		f = &pop2[j];
		p1 = NULL;
...
void Genloop_CAN(cromo *p1, cromo *p2, int prod, int cli, int* mcpo, int *prod_can, int *budgetpo, int *maxOffer, int** revenue, int** cost, int** profit, list** sorted_npp, float hurdle, float penaltie, int generations, int popSize, int *fcpo, float range, float hurdleTolerance, int** solc, int *solp, int* budget, float* sum_cost, float* sum_profit, int *active_clients, int *active_prod, bool new_pop, int *client_po, bool diversification, bool alternative_const, bool initialized)
{
	...
	pop2 = p2;
	//int min, mini;
	//Falta alocar e manter rastro dos produtos que estao sendo ofertados a partir do crossover_CAN
	//** Verificar se configuracao de produtos esta sendo devidamente transferida entre o genetico e o tabu
	...
	/*for(int c=0;c<prod;c++)
		printf("Budget for the product %d: %d \n", c, budget[c]);*/
	//If the genetic algorithm was not initialized, creates an initial population.
	if (!initialized)
	{

		//Tentar construir matrizes de mutacao para bloquear ofertas de mais baixo NPP comuns a maioria dos individuos.
		//**Problema está no client order.
		deviation = 0.05;
		list2 *aux_list, *prox;
		...
		//Calcular média de diferenca da media para ofertas acima da mediaaverage_npp
		float np = 0;
		for (j = 0; j < prod; j++)
		{
	...
	else //If the GA was already initialized, checks the flag for a new population creation. If it is true, generates a new population.
	{
		...
		if (new_pop)
		{
			...
			//Checar se o elite nao esta sendo colocador no lugar errado, tambem checar a funcao de complementacao no procedimento de limpeza
			//Ocorreu populacao com 0 de fitness, investigar no construtivo.
			elitei = maxi;
		...
		//check_values(&pop1[mini], fcpo, cost, revenue, pop1[mini].cost, pop1[mini].revenue, pop1[mini].fitness, prod, cli);
		//initialized = true;
		for (int c = 0; c < prod; c++)
		{
			budget[c] = pop1[mini].budget[c];
			//printf("Transfered solution budget for the product %d: %d \n", c, pop2[elitei].budget[c]);
	...
	for (gen = 0, popflag = true, sumGap = 0; gen < generations; gen++, popflag = !popflag)
	{
		...
		if (popflag)
		{
			...
			if (gen > -1)
			{
				...
				//*****Processo de transferencia de elite esta gerando erro no fitness
	...
	if (!popflag)
	{
		...
		for (int c = 0; c < prod; c++)
		{
			...
			//printf("Budget for the product %d: %d \n", c, pop2[return_solution].budget[c]);
	...
	else
	{
		...
		for (int c = 0; c < prod; c++)
		{
			...
			//printf("Budget for the product %d: %d \n", c, pop1[return_solution].budget[c]);
			if (pop1[return_solution].offeredProducts[c])
				active_prod[c] = 0;
		}
		//verify_solution(&pop1[return_solution], budgetpo, prod, cli, cost, revenue, mcpo, maxOffer, prod_can);
		//check_budget(&pop1[return_solution], budgetpo, cost, prod, cli);
		for (int c = 0; c < cli; c++)
		...
		*sum_profit = pop1[return_solution].revenue;
		...
		//check_values(&pop2[return_solution], fcpo, cost, revenue, pop2[return_solution].cost, pop2[return_solution].revenue, pop2[return_solution].fitness, prod, cli);
	}
	free(average_npp);
	free(probabilities);
}
